...
(Spring Session). The app is run with `make up` (compose files live in `deployments/`; see `README.md`).

This document answers the eight documentation points required by the assignment. Every claim
cites the real source location (`path:line`); the code is the source of truth.
...
### Backend — `config/` (security, session, web, observability)
| File | Responsibility |
|------|----------------|
...
**Cookie attributes** (`application.yml:57-63`): `http-only: true`, `secure: true`,
...
- **Idle timeout 15m** — `server.servlet.session.timeout: 15m` (`application.yml:63`); resets on activity.
- **Absolute timeout 8h** — `app.session.absolute-timeout: 8h` (`application.yml:5`), enforced by
...
repository, `application.yml:21-27`). A second login **expires the oldest** session; that session's
...
**`SameSite=strict`** (`application.yml:62`), which already prevents the browser from attaching
...
5. **Single-instance assumptions.** Some controls (the in-memory-style session/rate-limit reasoning)
   ...
   was not load-tested).
6. **No CSRF/anti-automation on a few non-mutating surfaces** and no WAF — out of scope for this
   assignment but worth stating.
